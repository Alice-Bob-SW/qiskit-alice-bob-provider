...
from qiskit_alice_bob_provider.processor.custom_logical_cat import (
    CustomLogicalCat,
)
...
def test_create_local_backend_with_custom_processor(
    ...
    # Given
    backend_name = 'EMU:CUSTOM_LOGICAL'

    # When
    backend = AliceBobLocalProvider().get_backend(
        backend_name, **default_custom_emulator_parameters
    )

    # Then
    assert backend is not None
    assert backend.name == 'EMU:CUSTOM_LOGICAL'
    assert backend._processor.n_qubits == 15
...
def test_missing_required_parameters(default_custom_emulator_parameters):
    ...
    with pytest.raises(
        ValueError,
        match="Backend parameters must include 'clock_cycle' and 'n_qubits'",
    ):
        CustomLogicalCat(**params)


def test_missing_noise_model_for_gate(default_custom_emulator_parameters):
    # Given
...
def test_supported_gates_list(default_custom_emulator_parameters):
    # Given
    processor = CustomLogicalCat(**default_custom_emulator_parameters)
...
def test_unsupported_gate_raises_error(default_custom_emulator_parameters):
    ...
    with pytest.raises(
        ValueError,
        match="Instruction 'rz' is not supported by CustomLogicalCat",
    ):
...
def test_invalid_noise_model_parameters(default_custom_emulator_parameters):
    # Given
    def invalid_noise_function(params, backend_params):
...
def test_invalid_time_model_parameters(default_custom_emulator_parameters):
    # Given
    def invalid_time_function(params, backend_params):
        return -1
...
def test_can_validate_custom_parameters(default_custom_emulator_parameters):
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Invalid parameters provided for CustomLogicalCat.'),
    ):
        processor = CustomLogicalCat(
            **default_custom_emulator_parameters,

...
def test_noise_on_initialize() -> None:
    circ = QuantumCircuit(3)
    circ.initialize('+', 0)
    circ.initialize('-', 1)
    circ.initialize('0', 2)
...
def test_pass_with_none_chi_matrix() -> None:
...
def test_chi_vs_pauli_errors() -> None:
    ...
    # Circuit with nonè diagonal chi matrix -> Kraus map
    circ = QuantumCircuit(10)
    circ.h(0)
    pm = PassManager(build_quantum_error_passes(SimpleProcessor()))
    transpiled = pm.run(circ)
